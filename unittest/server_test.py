...
@patch('psycopg2.connect')
def test_get_lesson(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = [
...
@patch('psycopg2.connect')
def test_get_lesson_invalid_id(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Invalid id") # Mock invalid id response
    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn

    response = client.get('/lesson/invalid_id')
    assert response.status_code == 400
    assert response.json == {'error': 'Invalid id'}

@patch('psycopg2.connect')
def test_get_tips(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = [
        (1, 1, "<b>Check the Sender's Email Address</b><br/> Phishers often use email addresses that resemble legitimate ones. Look closely for small variations, such as extra characters or misspellings.",
        )
    ]
    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
...
@patch('psycopg2.connect')
def test_get_tip_invalid_id(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Invalid id") # Mock invalid id response
    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    ...
    response = client.get('/tips/invalid_id')
    assert response.status_code == 400
    assert response.json == {'error': 'Invalid id'}
...
def test_get_score_invalid_id(mock_connect, client, login_and_get_token):
    # Mock the database connection and cursor
    mock_cursor = MagicMock()
    mock_cursor.fetchone.return_value = ('Invalid id')  # Mock invalid id response
    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn
...
def test_get_score_no_auth(mock_connect, client):
    # Mock the database connection and cursor
    mock_cursor = MagicMock()
    mock_cursor.fetchone.return_value = ('Authentication required') # Mock missing authentication
    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn
...
@patch('psycopg2.connect')
def test_update_points(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Points updated successfully for userid = 1") # Mock successful post request

    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn

    response = client.post('/update_points', json={
        "userid": 1,
        "topicid": 1,
        "points": 100,
    })
    assert response.status_code == 200
    assert response.json == {"message":"Points updated successfully for userid = 1"}

@patch('psycopg2.connect')
def test_update_points_invalid_id(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Invalid id") # Mock invalid id response
...
@patch('psycopg2.connect')
def test_update_points_missing_ids(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Missing userid or topicid") # Mock missing ids response

    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn

    response = client.post('/update_points', json={"points": 100})
    assert response.status_code == 400
    assert response.json == {"error":"Missing userid or topicid"}

@patch('psycopg2.connect')
def test_get_lesson_status(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = (True) # Mock successful get lesson status response

    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn

    response = client.get('/get_lesson_status/8/1')
    assert response.status_code == 200
    assert response.json == {"lesson_status":True}

@patch('psycopg2.connect')
def test_get_lesson_status_invalid_userid(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Invalid userid") # Mock invalid userid response

    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn

    response = client.get('/get_lesson_status/invalid_id/1')
    assert response.status_code == 400
    assert response.json == {"error":"Invalid userid"}

@patch('psycopg2.connect')
def test_get_lesson_status_invalid_topicid(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Invalid topicid") # Mock invalid topcid response

    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn

    response = client.get('/get_lesson_status/8/invalid_id')
    assert response.status_code == 400
    assert response.json == {"error":"Invalid topicid"}

@patch('psycopg2.connect')
def test_complete_lesson(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Lesson status updated successfully for userid = 19") # Mock successful complete lesson response

    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    mock_connect.return_value = mock_conn

    response = client.post('/complete_lesson', json={
        "userid": 19,
        "topcid": 1
    })
    assert response.status_code == 200
    assert response.json == {"mesage":"Lesson status updated successfully for userid = 19"}

@patch('psycopg2.connect')
def test_complete_lesson_missing_ids(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Missing userid or topicid") # Mock missing ids response
    ...
    response = client.post('/complete_lesson')
    assert response.status_code == 400
    assert response.json == {"error":"Missing userid or topicid"}

@patch('psycopg2.connect')
def test_complete_lesson_missing_userid(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Invalid userid") # Mock missing userid response
...
@patch('psycopg2.connect')
def test_complete_lesson_missing_topicid(mock_connect, client):
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = ("Invalid topic") # Mock missing topic response
    ...
    assert response.status_code == 400
    assert response.json == {"error":"Invalid topic"}